...
void ICPlocalization::setInitialPose(const Eigen::Vector3d &p,
	...
	std::cout << ", q: " << q.coeffs().transpose() << ", rpy: "
			<< kRadToDeg * icp_loco::toRPY(q).transpose() << " deg \n";
	lastPosition_ = p;
	lastOrientation_ = q;
...
void ICPlocalization::initializeInternal() {
	...
	tfPublisher_ = std::make_shared<TfPublisher>(nh_, frameTracker_,
			imuTracker_);

	lastPosition_.setZero();
	lastOrientation_.setIdentity();

	registeredCloudPublisher_ = nh_.advertise<sensor_msgs::PointCloud2>(
			"registered_cloud", 1, true);
...
void ICPlocalization::initialize() {
	...
	const std::string configFileFilters = nh_.param<std::string>(
	...
	if (!nh_.param<std::string>("icp_localization/range_data_topic",
			rangeDataTopic, "")) {
	...
	if (!nh_.param<std::string>("icp_localization/odometry_data_topic",
			odometryDataTopic, "")) {
	...
	const Rigid3d imuToRangeSensor = Rigid3d(
			getPositionFromParameterServer(nh_, imuLidarPrefix),
	...
	const Rigid3d odometrySourceToRangeSensor = Rigid3d(
			getPositionFromParameterServer(nh_, odometrySourceLidarPrefix),
	...
	rangeDataAccParam.numAccumulatedRangeData_ = nh_.param<int>(
			"icp_localization/num_accumulated_range_data", 1);
	rangeDataAccParam.inputRangeDataTopic_ = nh_.param<std::string>(
			"icp_localization/range_data_topic", "");
...
void ICPlocalization::matchScans() {
	...
	if (!isFirstScanMatch_) {
		...
		const Rigid3d motionCorrectedPose = Rigid3d(initPosition,
				initOrientation) * motionPoseChange;
...
//  std::cout << "Ground truth diff: " << poseDiffIcp.asString() << "\n\n";
	...
	frameTracker_->setTransformMapToRangeSensor(
			TimestampedTransform { optimizedPoseTimestamp_, Rigid3d(
					lastPosition_, lastOrientation_) });
...
void ICPlocalization::publishPose() const {
	...
	pose_msg.pose = pointmatcher_ros::eigenMatrixToPoseMsg<float>(
			optimizedPose_);
...
void ICPlocalization::publishRegisteredCloud() const {
	...
	sensor_msgs::PointCloud2 ros_msg =
			pointmatcher_ros::pointMatcherCloudToRosMsg<float>(data_out,
					fixedFrame_, ros::Time::now());
...
void ICPlocalization::icpWorker() {
	...
	while (ros::ok()) {
		...
		const unsigned int nUs = ch::duration_cast<ch::microseconds>(
